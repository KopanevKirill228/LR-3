    ...
    const std::string& fromName, const std::string& toName)
{
    T disk = from.Pop();
    to.Push(disk);
    moves_.Append({ fromName, toName, (int)disk });
}
...
void Hanoi<T>::Solve() {
    ...
    int n = (int)pegA_.GetCount();
    Solve(n, pegA_, pegB_, pegC_, nameA_, nameB_, nameC_);
}

template <typename T>
const MutableArraySequence<Move>& Hanoi<T>::GetMoves() const { return moves_; }
...
template <typename T>
int Hanoi<T>::GetMaxDisk() const { return maxDisk_; }

template <typename T>
int Hanoi<T>::GetDiskCount() const { return (int)pegA_.GetCount(); }